...
- 独立性要求：家庭背景必须完全独立，体现该角色自己的成长环境和家庭故事
- 考虑因素：根据角色的年龄（${memberData.age}岁）、职业（${memberData.occupation}）、与主角的关系（${memberData.relation}）来构思
- 参考方向：
  * 如果是妻子/丈夫：描述其原生家庭（结婚前的家庭），与主角家庭完全不同
  * 如果是女儿/儿子：如果年龄已成年，可以描述其独立工作后的生活背景
  * 如果是父母：可以描述他们的家族历史、职业背景等，与主角当前家庭区分开
  * 如果是朋友/同事：必须描述完全不同的家庭背景，没有任何共同点
- 示例：如果主角家庭是"普通工薪家庭"，该角色的家庭背景可以是"书香门第"、"商人世家"、"农民家庭"等，但不能是"相似的家庭"
