...
## 工作流程规范（重要 - 2026年5月13日更新）

### 正确工作流程（强制执行）

#### AI需要做的事：
1. **分析问题**：在用户提出问题后，分析用户目的，并给出建议
2. **沟通确认**：和用户沟通后再确认改动，并检查改动是否会对已经实现的功能造成影响
3. **推送代码**：在做完修改后，**主动推送至github仓库**
4. **告知用户**：告知用户 commit 哈希值，简述所做的改动

#### 用户需要做的事：
1. 接收到 commit 哈希值后**手动到 Render 后台进行手动部署**确认最新代码
2. 部署完成后进行相关功能测试
3. 给出反馈，形成循环
...
3. **Render 部署地址**
   - Dashboard: https://dashboard.render.com/web/srv-d7bjmb7afjfc73f0qttg
   - 服务名称: KylinAI-1
...
## 小说专属角色数据功能（2025-05-03）

### 核心功能
1. 每个小说有独立的角色数据副本
2. 角色包含社会经历字段（短期经历 + 长期记忆）
3. 角色记忆管理机制：AI读取参考，定期整合
...
interface CharacterData {
  // ... 原有字段
  short_term_memory: string[];  // 短期经历
  long_term_memory: string[];   // 长期记忆
}
```

### 核心流程
1. 用户点击续写 → AI读取角色背景 → 创作
2. 续写完成后，记录这一次涉及的"经历"到短期经历
3. 当短期经历累积到10条，触发"记忆整合"
4. AI将多条短期经历整合成1-2条精炼的长期记忆
5. 清空短期经历，保留长期记忆
6. 下次续写时，AI读取长期记忆，保持角色一致性

### 页面入口
- 正在创作页面右上角添加"小说数据库"按钮
- 点击进入小说数据库页面，查看所有数据和设定

### API
- POST /api/v1/experience/summarize-experiences - AI整合经历
